...
- 作者/维护者：仓库贡献者
- 适用 OpenFOAM 版本：用户环境下的 OpenFOAM v2412（或兼容的 v2x+）
...
## 关键实现流程（每步的数据流与要点）
## 故障排查与调优建议
## 附录：示例 dynamicMeshDict、controlDict 片段
...
在 case 的 constant/dynamicMeshDict 中启用方法：
...
设置/读取
- void readControls()
  - 作用：从 dynamicMeshDict 读取控制项，初始化 couplingRelaxation_、faceDiagnosticsMode_、mappingTolerance_、modalFilesPath 等。
  - 关键变量：couplingRelaxation_（double）、faceDiagnosticsMode_（int）、mappingTolerance_（double）、diagnosticsDir_（string）

- void readModeShapes()
  - 作用：解析模态文件（如 FluidNodeDisp*.csv），读取 modalShapes_（vector<vector<double>> 或 scalarField 的集合）、modalFrequencies_（vector<double>）、nModes_、nNodes_。
  - 关键变量：modalShapes_（每模态的节点位移向量集合）、modalFrequencies_（角频率或频率）、modalMass_（可选读取或默认 1.0）、modalDamping_（可选或默认 0.0）

映射与几何
- void buildMapping()
  - 作用：将结构节点（CSV）映射到流体网格节点/面。生成 fluidNodeMap_（structureNodeID -> meshPointID）和 faceToNodeMap_ 以便后续位移重建。
  - 关键变量：fluidNodeMap_（map）、mappingTolerance_（double）

力投影与积累
- void calcModalForces(const scalar time)
  - 作用：对当前流体场（压力、剪切力）在 fsiPatches 上的每个面进行遍历，将面力投影到模态基底，分别累加到 modePressureForce_[m] 与 modeShearForce_[m]。
  - 关键变量：modePressureForce_（vector<vector<scalar>> 或 vector<scalar> 随模态）、modeShearForce_；face-level临时变量：facePressure, faceShear, faceAreaVec
  - 注意：代码内部对面法向以及剪切投影使用精确的力分解公式，确保压力贡献与剪切贡献分离。

结构动力学求解
- void solveStructuralDynamics(scalar dt)
  - 作用：对每个模态 m 求解 M_m * x_dd + C_m * x_d + K_m * x = F_m，其中 K_m 约定为 modalFrequencies_[m]^2（若模态频率是角频率），M_m、C_m 从输入或默认值。数值积分方法：可能使用 Wilson-theta 或 Newmark（实现中为 Wilson-theta 单步公式，参阅代码注释）。
  - 关键变量：appliedModeDisp_（每模态经松弛后实际应用的位移，用于诊断）、modeDisp_（当前模态位移）、modeVel_、modeAcc_、modalMass_、modalDamping_
  - 注意：若 modalMass_/modalDamping_ 未提供，默认值会影响物理正确性，建议在输入文件中指定。

网格更新与写出
- void applyRelaxationAndUpdate()
  - 作用：将结构求解产生的模态位移按 couplingRelaxation_ 与上一时刻 appliedModeDisp_ 混合，得到新的 appliedModeDisp_ = alpha*newDisp + (1-alpha)*oldApplied。然后从模态位移重建为节点位移并调用 movePoints()。
  - 关键变量：couplingRelaxation_（alpha, double）、appliedModeDisp_（vector）
...
## 关键实现流程（每步的数据流与要点）
初始化（构造中）：
- 读控件 -> 读模态文件（modalShapes_ + modalFrequencies_）-> 建立结构节点到流体网格的映射（fluidNodeMap_）-> 打开诊断文件。

每个时间步（update）:
## 遍历 fsiPatches 中的每个面：读取 pressure、wallShearStress（或基于速度梯度计算剪切），计算面力向量。
## 将每个面力按面作用点插值到映射到的结构节点（或直接以面贡献投影到模态基底）。
## 对所有节点/面贡献执行模式投影：F_m = sum_{nodes} phi_m^T * f_node，分别记录 pressure 与 shear 的分量。
## 调用 solveStructuralDynamics(dt) 得到 modeDisp_, modeVel_, modeAcc_。
## 调用 applyRelaxationAndUpdate()：appliedModeDisp_ = alpha*modeDisp + (1-alpha)*appliedModeDisp_previous。
## 重建节点位移并调用 movePoints()，更新网格几何。
## 写出 modal_diagnostics.csv 与可选 faceDiagnostics 文件。


## 故障排查与调优建议
常见错误与修复建议：
- 映射失败（某些结构节点找不到流体对应点）：检查 FluidNodeCoor.csv 的坐标系与单位是否匹配流体网格，增加 mappingTolerance。
- FPE 或求解器崩溃（GAMG/PCG 遇到非数）：通常是耦合不稳定导致压力失控。策略：降低 couplingRelaxation（例如 0.1）、增加模拟阻尼 modalDamping_、或为模态指定更大的 modalMass_。对于严重情况，建议实现 Aitken 或子迭代耦合。
...
如需将本手册翻译为英文或导出为 HTML/PDF，可再次提出请求。
